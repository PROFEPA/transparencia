...
const glosarioTerms: GlosarioTerm[] = [
  ...
  { termino: "Avance", definicion: "Valor alcanzado del indicador en un momento determinado. El avance se compara contra la meta para determinar el porcentaje de cumplimiento." },
  {
  ...
  { termino: "Fuente de verificación", definicion: "Documento, sistema o registro que contiene la información para verificar el valor del indicador. Permite la auditoría y comprobación de los datos reportados." },
  { termino: "PROFEPA", definicion: "Procuraduría Federal de Protección al Ambiente. Es un órgano administrativo desconcentrado de la Secretaría de Medio Ambiente y Recursos Naturales (SEMARNAT), encargado de vigilar el cumplimiento de las disposiciones legales aplicables en materia ambiental." },
  { termino: "Trazabilidad", definicion: "Capacidad de identificar el origen y la ruta que siguió un dato desde su fuente original hasta su presentación final. En este tablero, cada indicador incluye información sobre el archivo y la ubicación exacta de donde fue extraído." },
  { termino: "Datos abiertos", definicion: "Datos que pueden ser libremente utilizados, reutilizados y redistribuidos por cualquier persona. Los datos en este tablero se publican en formatos abiertos (CSV, JSON) para promover la transparencia y el análisis independiente." },
];
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-wrap gap-2">
            {letters.map(letter => (
              <a key={letter} href={`#letra-${letter}`}
                className="w-10 h-10 flex items-center justify-center rounded-lg bg-gray-100 hover:bg-gob-green-500 hover:text-white font-bold transition-colors">{letter}</a>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-8">
          {letters.map(letter => (
            <section key={letter} id={`letra-${letter}`}>
              <h2 className="text-2xl font-bold text-gob-green-600 mb-4 pb-2 border-b">{letter}</h2>
              <div className="space-y-4">
                {groupedTerms[letter].map((term, index) => (
                  <motion.div key={index} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-30px' }} transition={{ duration: 0.3, delay: index * 0.03 }}
        ...
        <div className="mt-8 text-center">
          <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
            className="inline-flex items-center gap-2 text-gob-green-600 hover:underline">
